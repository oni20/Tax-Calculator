                ...
                <div>  
                    {Object.keys(props.tableBody).map((body, index) => {
                        return (
                            ...
                            <p className={`${SalaryContainerStyles.income__amount} ${body === 'annual' ? SalaryContainerStyles.income__highlight : ''} `} key={index}> {body !== 'income' && body !== 'annual' && body !== 'rrspsavings' ? '-' : ''} {body === 'rrspsavings' ? '+' : ''} $ {props.tableBody[body]}</p>   
                        );
                    })}
                </div>
            </div>
